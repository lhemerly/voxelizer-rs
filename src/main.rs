...
struct Args {
    ...
    /// Path to destination file. Format inferred from extension (.csv, .ply, .vtk, .obj, .vox, or .bin)
    #[arg(short, long)]
    output: String,
    ...
    /// Optional flag. Output will only contain surface voxels instead of solid filling
    #[arg(long)]
    surface_only: bool,

    /// Evaluate and store only voxels where absolute distance is within desired threshold (e.g. 1.0)
    ...
    /// Optional flag. Mesh will be centered at the origin (0, 0, 0) before processing
    #[arg(long)]
    center: bool,

    /// Translates the mesh by 'x,y,z' offsets (e.g., 10,-5,0)
    #[arg(long, value_parser = parse_vec3)]
    translate: Option<[f64; 3]>,

    /// Rotates the mesh by 'x,y,z' degrees (e.g., 90,0,0)
    #[arg(long, value_parser = parse_vec3)]
    rotate: Option<[f64; 3]>,
    ...
    /// Specify a sphere for assigning a phase 'x,y,z,radius' (e.g., 0,0,0,5)
    #[arg(long, value_parser = parse_vec4)]
    phase_sphere: Option<[f64; 4]>,

    /// Number of parallel threads to use. Defaults to system's logical core count
    #[arg(long)]
