...
struct Args {
    ...
    /// Path to destination file. The format is inferred from the extension.
    #[arg(short, long)]
    output: String,
    ...
    /// Optional value. Evaluate and store only the voxels where the absolute distance is within the desired threshold.
    ...
    /// Crops the voxelization bounding box (the evaluated spatial volume) to 'min_x,min_y,min_z,max_x,max_y,max_z'.
