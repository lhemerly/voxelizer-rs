...
### Multi-format Export: Outputs to PLY, CSV, VTK, OBJ, MagicaVoxel (.vox), XYZ (.xyz), or a compact binary (.bin) format depending on the file extension.
...
``` -o, --output ```: Path to destination file. The format is inferred from the extension (`.csv`, `.ply`, `.vtk`, `.obj`, `.vox`, `.xyz`, or `.bin`).
